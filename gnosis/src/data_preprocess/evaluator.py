...
# Define colors
    ...
    with timing(f"plot:{name}:raw"):
        ...
        # Note: I've kept 'density=True' to ensure the Y-axis is mathematically 
        # normalized (area sums to 1) as you requested.
        
        if prob_like:
            rmin, rmax = 0.0, 1.0
            plt.hist(s_raw[y == 1], bins=bins, range=(rmin, rmax), alpha=0.6, density=True,
                     label="Correct", color=COLOR_CORRECT_DARK)
            plt.hist(s_raw[y == 0], bins=bins, range=(rmin, rmax), alpha=0.6, density=True,
                     label="Wrong", color=COLOR_WRONG_DARK)
        else:
            rmin, rmax = np.min(s_raw), np.max(s_raw)
            if np.isfinite(rmin) and np.isfinite(rmax) and rmin < rmax:
                plt.hist(s_raw[y == 1], bins=bins, range=(float(rmin), float(rmax)), alpha=0.6, density=True,
                         label="Correct", color=COLOR_CORRECT_DARK)
                plt.hist(s_raw[y == 0], bins=bins, range=(float(rmin), float(rmax)), alpha=0.6, density=True,
                         label="Wrong", color=COLOR_WRONG_DARK)
            else:
                plt.hist(s_raw[y == 1], bins=bins, alpha=0.6, density=True,
                         label="Correct", color=COLOR_CORRECT_DARK)
                plt.hist(s_raw[y == 0], bins=bins, alpha=0.6, density=True,
                         label="Wrong", color=COLOR_WRONG_DARK)
        
        # --- UPDATED LABELS ---
        
        # X-Axis: The score the model gave (0.0 to 1.0)
        plt.xlabel("Predicted Correctness Score", fontsize=16) 
        
        # Y-Axis: The normalized volume (0.0 to 1.0)
        plt.ylabel("Proportion", fontsize=16)
        
        # Clean title
        clean_name = name.replace("_", " ").title()
        plt.title(f"Distribution of {clean_name}", fontsize=16)
        ...
        raw_path = out_dir / f"{name}_distribution_raw.png"
        plt.tight_layout(); plt.savefig(raw_path, dpi=300); plt.close()
